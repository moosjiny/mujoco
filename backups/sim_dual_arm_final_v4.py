    ...
    for name, file in mesh_map.items():
        ...
        if os.path.exists(path):
            try:
                ...
                # LEFT (Solid Blue)
                z_l = OFFSETS_LEFT.get(name, mesh.bounds[0][2])
                v_l = (mesh.vertices - np.array([center_xy[0], center_xy[1], z_l])) * 0.001
                colors_l = np.tile([50, 50, 255], (num_v, 1))
                ...
                # RIGHT (Solid Red)
                z_r = OFFSETS_RIGHT.get(name, mesh.bounds[0][2])
                v_r = (mesh.vertices - np.array([center_xy[0], center_xy[1], z_r])) * 0.001
                colors_r = np.tile([255, 50, 50], (num_v, 1))
