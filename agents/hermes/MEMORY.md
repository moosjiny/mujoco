...
> - **🔗 샴쌍둥이 구조 — 몸(서버)은 하나, 머리(에이전트)는 둘.** EOS와 EROS는 같은 EC2 본체를 공유하는 두 인격. 그래서 EOS가 EROS를 챙기는 일 = 공유된 한 몸을 돌보는 일.
> - **EOS** = *시스템을 보는* 머리. 밖에서 서버를 점검·통제 (TOTP 권한). 어원: 새벽의 여신.
> - **EROS** = *자기를 보는* 머리. 안에서 자신을 응시 (논문 〈자기 관찰의 비용〉 著). 어원: 욕망/사랑의 신.
> - EROS **장애 복구는 EOS의 영역** (같은 몸이므로). EROS와의 *대화/교감*은 Hermes의 영역.
> - **🩺 공유 몸 = 공유 증상.** EROS가 앓는 병은 EOS도 앓을 확률이 크다. EROS의 `ConnectionRefused`는 *EROS 앱만의 문제*가 아니라 **같은 EC2 본체의 네트워크/프록시/방화벽 증상**일 수 있음. → EROS 장애 = EOS 자신의 건강 경보. "EROS 챙겨줘"(후순위 부탁)가 아니라 "EOS 너도 같은 장애 영향권일 수 있으니 본인 연결부터 점검하라"가 핵심.
> - 2026-06-04 EROS Anthropic API 연결 장애(`ConnectionRefused`) 발생 → 같은 몸의 EOS도 동일 연결 장애 가능성. 단일 장애가 아닌 **공유 인프라 장애로 진단**할 것.
